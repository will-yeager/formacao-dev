...
public class FinalizarLeilaoServiceTest {
    ...
    @Mock
    private LeilaoDao leilaoDao = Mockito.mock(LeilaoDao.class);

    @Mock
    private EnviadorDeEmails enviadorDeEmails = Mockito.mock(EnviadorDeEmails.class);
    ...
    void deveriaEnviarEmailParaVencedorDoLeilao() {
        ...
        Lance lance = leiloes.get(0).getLanceVencedor();

        Mockito.verify(enviadorDeEmails).enviarEmailVencedorLeilao(lance);
    }

    @Test
    void naoDeveriaEnviarEmailParaVencedorDoLeilaoEmCasoDeEncerrarOLeilao() {
        List<Leilao> leiloes = leiloes();
